...
| Header forwarding | `inference.local` forwards only a per-provider header allowlist. OpenAI routes allow `openai-organization` and `x-model-id`. Anthropic routes allow `anthropic-version` and `anthropic-beta`. Vertex Claude rawPredict routes strip `anthropic-beta` and do not forward `anthropic-version` as a header because the router injects `anthropic_version` into the Vertex request body. NVIDIA routes allow `x-model-id`. Codex and Grok subscription routes allow no caller passthrough headers; Codex injects its gateway-owned routing headers. AWS Bedrock routes have no passthrough headers today. All other caller headers are stripped. |
| Configuration | A workspace route supplies the default provider/model. A sandbox can instead pin an attached provider/model at creation time. Local selection is explicit and never depends on attachment order. |
| Provider support | NVIDIA, Anthropic, Google Vertex AI, AWS Bedrock (via a translating bridge — direct AWS with SigV4 signing is a separate follow-up), the experimental gateway-managed Codex and Grok subscription providers, and any OpenAI-compatible provider all work through the same endpoint. Vertex routes Claude models through `/v1/messages` and non-Anthropic models through `/v1/chat/completions`. The gateway resolves the upstream Vertex host from the provider config, including regional, global, and supported multi-region endpoints. |
...
| Hot refresh | OpenShell picks up provider credential changes and workspace-route updates without recreating sandboxes. Changes propagate within about 5 seconds by default. A sandbox-local provider/model choice is immutable sandbox spec and is changed by recreating the sandbox. |
...
The experimental personal-subscription adapters expose only their reviewed
surfaces:

| Provider | Pattern | Method | Sandbox path |
|---|---|---|---|
| OpenAI Codex | Responses | `POST` | `/v1/responses` |
| xAI Grok | Chat Completions | `POST` | `/v1/chat/completions` |
| xAI Grok | Model discovery | `GET` | `/v1/models` and `/v1/models/*` |

Their credential-bearing upstream origins cannot be overridden. Unsupported
OpenAI-compatible paths are denied instead of being inferred from the broader
...
There are two deterministic selection modes:

- A workspace default created with `openshell inference set` applies when a
  sandbox has no local selection.
- Paired `--inference-provider` and `--inference-model` flags select one
  provider/model for a new sandbox. The exact provider must also be attached
  with `--provider`.

OpenShell never uses provider attachment order. If the selected provider is
missing, unattached, expired, revoked, or awaiting reauthentication, the route
is unavailable and does not fall through to another attachment.
...
## Set Workspace-Default Inference Routing
...
## Set Sandbox-Local Inference Routing

Select an attached provider and model when creating a sandbox:

```shell
openshell sandbox create \
  --name personal-model-agent \
  --provider grok-subscription \
  --inference-provider grok-subscription \
  --inference-model grok-4.6
```

Both inference flags are required together. This selection overrides the
workspace default for only that sandbox. Use it when personal subscription
providers coexist or different sandboxes need different models.
...
- Deterministic scope: Sandboxes without a local choice use the workspace route; sandboxes with a local choice use exactly that attached provider/model.
- HTTPS only: `inference.local` is intercepted only for HTTPS traffic.
- Hot reload: Credential and workspace-route changes are picked up by running sandboxes within about 5 seconds by default. Changing sandbox-local selection requires sandbox recreation.
