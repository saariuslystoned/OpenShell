...
Do not combine `protocol: tcp` with `access`, `rules`, or `deny_rules`. Those fields require an inspected L7 protocol. A runtime that cannot provide policy DNS and transparent TCP capture rejects the policy instead of silently allowing a weaker proxy-only path.
