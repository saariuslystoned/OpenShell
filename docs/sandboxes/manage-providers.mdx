...
| `openai-codex-oauth` | None | Experimental gateway-owned Sign in with ChatGPT grant. The access and refresh tokens are never injected. Refer to [OpenAI Codex Subscription](/providers/openai-codex-subscription). |
| `opencode` | `OPENCODE_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_API_KEY` | OpenCode |
| `xai-grok-oauth` | None | Experimental gateway-owned xAI Grok grant. The access and refresh tokens are never injected. Refer to [xAI Grok Subscription](/providers/xai-grok-subscription). |
...
| xAI Grok subscription (experimental) | `grok-subscription` | `xai-grok-oauth` | `https://api.x.ai/v1` (pinned) | None; separate gateway-managed xAI device grant |
...
Refer to your provider's documentation for the correct base URL, available models, and API key setup. For gateway-managed personal grants, refer to [OpenAI Codex Subscription](/providers/openai-codex-subscription) and [xAI Grok Subscription](/providers/xai-grok-subscription). For the Vertex-specific auth flows and config keys, refer to [Google Vertex AI](/providers/google-vertex-ai). To configure inference routing, refer to [Inference Routing](/sandboxes/inference-routing).
