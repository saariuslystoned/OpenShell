...
be usable without OpenShell creating it or changing its ownership or mode. The
completed UID/GID and supplementary groups must be able to traverse every
parent and write and enter the effective workspace.

Before loading policy, credentials, TLS, or networking state, the final
supervisor rejects symlink components, non-directories, kernel-managed
filesystems, and OpenShell control paths. The drivers reject overlap in either
...
it does not validate the integrity of a custom image. Before workload launch,
a one-shot validator drops to the final identity and repeats the safe walk with
kernel effective-access checks, including POSIX ACL grants and LSM denials.
Image `VOLUME` declarations that mask the workdir or one of its parents are
...
to use `WORKDIR`; failures surface before readiness. Rootless, rootful,
user-namespace, and SELinux configurations may initialize volumes differently.
